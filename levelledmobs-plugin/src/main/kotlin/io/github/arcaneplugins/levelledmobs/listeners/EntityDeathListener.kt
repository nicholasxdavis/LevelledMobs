...
class EntityDeathListener : Listener {
    ...
    private fun onDeath(event: EntityDeathEvent) {
        ...
        val scheduler = SchedulerWrapper(event.entity){
            // Handle mob stacking - check if entity is stacked
            var stackMultiplier = 1
            var shouldCancelDeath = false
            
            if (main.stackManager.isStackingEnabled() && main.stackManager.isStacked(event.entity)) {
                val stackSize = main.stackManager.getStackSize(event.entity)
                
                if (stackSize > 1) {
                    // Handle stack splitting
                    val result = main.stackManager.handleStackDeath(event.entity)
                    stackMultiplier = result.first
                    shouldCancelDeath = result.second
                }
            }
            ...
            if (lmEntity.isLevelled) {
                ...
                if (stackMultiplier > 1 && !doNotMultiplyDrops) {
                    val originalDrops = event.drops.toList()
                    for (i in 1 until stackMultiplier) {
                ...
                if (disableXpDrops)
                    ...
                    if (event.droppedExp > 0) {
                        val baseExp = main.levelManager.getLevelledExpDrops(lmEntity, event.droppedExp.toDouble())
                        // Multiply XP by stack size
                        event.droppedExp = (baseExp * stackMultiplier).toInt()
                    }
                }
                ...
                // Multiply custom drops by stack size
                if (stackMultiplier > 1) {
                    val originalDrops = drops.toList()
                    for (i in 1 until stackMultiplier) {
                        drops.addAll(originalDrops.map { it.clone() })
                    }
                }

                event.drops.addAll(drops)
            } else if (stackMultiplier > 1) {
                // Even if not levelled, multiply vanilla drops by stack size
                val originalDrops = event.drops.toList()
                for (i in 1 until stackMultiplier) {
                    for (drop in originalDrops) {
                        event.drops.add(drop.clone())
                ...
                event.droppedExp = (event.droppedExp * stackMultiplier).coerceAtLeast(0)
            }
            
            // Cancel death if stack remains and skip animation is enabled
            if (shouldCancelDeath) {
                event.isCancelled = true
            }
            
            // Clean up stack display on death
            if (main.stackManager.isStackingEnabled()) {
                main.stackManager.removeStackDisplayOnDeath(event.entity)
            }
