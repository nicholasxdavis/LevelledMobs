...
class LevelManager : LevelInterface2 {
    ...
    private fun replaceStringPlaceholders(
        ...
        val stackSize = if (LevelledMobs.instance.stackManager.isStackingEnabled()) {
            LevelledMobs.instance.stackManager.getStackSize(lmEntity.livingEntity)
        } else {
            1
        }
        val stackDisplay = if (stackSize > 1) " &7x$stackSize" else ""
        text.replace("%stack-size%", stackDisplay)
        text.replace("%stack%", if (stackSize > 1) stackSize.toString() else "")
